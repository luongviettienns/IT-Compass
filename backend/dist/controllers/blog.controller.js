...
const asBlogPostStatus = (value) => value === 'DRAFT' || value === 'SCHEDULED' || value === 'PUBLISHED' ? value : undefined;
const asCommentListStatus = (value) => value === 'all' || value === 'deleted' || value === 'visible' || value === 'hidden' ? value : undefined;
...
export const adminBlogStats = asyncHandler(async (_req, res) => {
    const stats = await blogService.adminBlogStats();
    return res.status(200).json({ stats });
});
export const adminListBlogAuditLogs = asyncHandler(async (req, res) => {
    const result = await listAdminAuditLogs(req.query);
    return res.status(200).json(result);
});
export const adminCheckSlug = asyncHandler(async (req, res) => {
    const query = req.query;
...
export const adminGetPostById = asyncHandler(async (req, res) => {
...
export const updateBlogPostStatus = asyncHandler(async (req, res) => {
    const actor = requireAuthenticatedUser(req);
    const body = req.body;
...
export const scheduleBlogPost = asyncHandler(async (req, res) => {
    const actor = requireAuthenticatedUser(req);
    const body = req.body;
...
export const restoreBlogPost = asyncHandler(async (req, res) => {
    const actor = requireAuthenticatedUser(req);
    const post = await blogService.restoreBlogPost({ id: String(req.params.id), actorId: actor.id });
    return res.status(200).json({ post });
});
export const bulkDeleteBlogPosts = asyncHandler(async (req, res) => {
    const actor = requireAuthenticatedUser(req);
    const body = req.body;
...
export const bulkRestoreBlogPosts = asyncHandler(async (req, res) => {
    const actor = requireAuthenticatedUser(req);
    const body = req.body;
...
export const bulkPublishBlogPosts = asyncHandler(async (req, res) => {
    const actor = requireAuthenticatedUser(req);
    const body = req.body;
...
export const previewBlogPost = asyncHandler(async (req, res) => {
    const post = await blogService.previewBlogPost({ id: String(req.params.id) });
    return res.status(200).json({ post });
});
export const listCommentsBySlug = asyncHandler(async (req, res) => {
