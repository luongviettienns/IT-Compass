...
const ADMIN_AUDIT_ACTION_FROM_DB = Object.fromEntries(Object.entries(ADMIN_AUDIT_ACTION_TO_DB).map(([appValue, dbValue]) => [dbValue, appValue]));
...
const mapActionToDb = (action) => {
    const mapped = ADMIN_AUDIT_ACTION_TO_DB[action];
    if (!mapped) {
...
const buildWhereSql = ({ actorUserId, action, targetType, targetId }) => {
    ...
    if (action) {
        conditions.push(Prisma.sql `a.hanh_dong = ${mapActionToDb(action)}`);
    }
    if (targetType) {
        conditions.push(Prisma.sql `a.loai_doi_tuong = ${targetType}`);
    }
    if (targetId) {
...
const normalizeAuditLog = (log) => ({
    ...
    actorUser: log.actorUserId
        ? {
            ...
            role: USER_ROLE_FROM_DB[log.actorRole] || log.actorRole,
            status: USER_STATUS_FROM_DB[log.actorStatus] || log.actorStatus,
        }
        : null,
});
export const createAdminAuditLog = async ({ actorUserId, action, targetType, targetId, reason, metadata, }) => {
