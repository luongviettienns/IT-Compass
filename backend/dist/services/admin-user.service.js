...
const buildDateRange = (createdFrom, createdTo) => {
    ...
    if (createdTo)
        range.lte = new Date(createdTo);
    return range;
};
const buildUserOrderBy = ({ sortBy = 'createdAt', sortOrder = 'desc' }) => {
    if (sortBy === 'fullName' || sortBy === 'email') {
        return [{ [String(sortBy)]: sortOrder }, { createdAt: 'desc' }];
    }
    return [{ [String(sortBy)]: sortOrder }, { fullName: 'asc' }];
};
const buildAuditOrderBy = () => [{ createdAt: 'desc' }, { id: 'desc' }];
...
const serializeAdminUser = (user, extra = {}) => {
    ...
    return {
        ...
        ...Object.fromEntries(Object.entries(extra).filter(([key]) => key !== 'activeSessionCount' && key !== 'lastSessionAt')),
    };
};
const createAuditLog = async ({ actorUserId, targetUserId, action, reason, before, after, }) => prisma.userAdminAuditLog.create({
    data: {
        actorUserId,
        targetUserId,
        action,
        ...
        afterJson: (after ?? Prisma.JsonNull),
    },
});
...
const ensureTargetIsNotProtectedAdmin = async (targetUserId) => {
...
const buildUserWhere = ({ search, role, status, emailVerified, hasProfile, createdFrom, createdTo, }) => {
    const trimmedSearch = typeof search === 'string' ? search.trim() : undefined;
    const createdAt = buildDateRange(createdFrom, createdTo);
    return {
        ...(role !== 'all' ? { role } : {}),
        ...(status !== 'all' ? { status } : {}),
        ...(emailVerified === 'true'
...
const mapBulkResult = ({ userId, success, code, message, user, revokedCount }) => ({
...
export const adminListUsers = async (input) => {
    ...
    const serializedUsers = users.map((user) => serializeAdminUser(user, {
...
const getActiveSessionCount = async (userId) => prisma.authSession.count({
    where: {
        userId,
        revokedAt: null,
        expiresAt: { gt: new Date() },
...
const getLastSessionAt = async (userId) => {
...
export const adminUpdateUserAccount = async ({ id, actorId, fullName, email, emailVerified, reason, }) => {
    ...
    const updatedUser = await prisma.user.update({
        ...
        data: {
            ...
            ...(email !== undefined
                ? {
                    email,
                    emailVerifiedAt: email === existingUser.email
...
export const adminUpdateUserProfile = async ({ id, actorId, reason, ...profileData }) => {
    const targetUserId = toBigIntId(id, 'user id');
    await ensureTargetIsNotProtectedAdmin(targetUserId);
    const existingUser = await getAdminUserById(id);
    const before = snapshotUser(existingUser);
...
export const adminUpdateUserStatus = async ({ id, actorId, status, reason, }) => {
    const targetUserId = toBigIntId(id, 'user id');
    await ensureTargetIsNotProtectedAdmin(targetUserId);
    const existingUser = await getAdminUserById(id);
    const before = snapshotUser(existingUser);
...
export const adminUpdateUserRole = async ({ id, actorId, role, reason, }) => {
    const targetUserId = toBigIntId(id, 'user id');
    await ensureTargetIsNotProtectedAdmin(targetUserId);
    const existingUser = await getAdminUserById(id);
    const before = snapshotUser(existingUser);
...
export const adminRevokeUserSessions = async ({ id, actorId, reason, }) => {
    const targetUserId = toBigIntId(id, 'user id');
    await ensureTargetIsNotProtectedAdmin(targetUserId);
    const existingUser = await getAdminUserById(id);
    const before = {
...
export const adminBulkUpdateStatus = async ({ actorId, userIds, status, reason, }) => {
...
export const adminBulkRevokeSessions = async ({ actorId, userIds, reason, }) => {
    const actorBigIntId = toBigIntId(actorId, 'actor id');
    const results = [];
...
export const adminListAuditLogs = async ({ page = 1, limit = 20, actorUserId, targetUserId, action, createdFrom, createdTo, }) => {
    const parsedPage = normalizePositiveInt(page, 1);
    const parsedLimit = normalizePositiveInt(limit, 20);
    const createdAt = buildDateRange(createdFrom, createdTo);
    const where = {
        ...(actorUserId ? { actorUserId: toBigIntId(actorUserId, 'actor user id') } : {}),
        ...(targetUserId ? { targetUserId: toBigIntId(targetUserId, 'target user id') } : {}),
        ...(action ? { action } : {}),
        ...(createdAt ? { createdAt } : {}),
    };
...
export const adminListUserAuditLogs = async ({ id, page = 1, limit = 20, action, createdFrom, createdTo, }) => {
