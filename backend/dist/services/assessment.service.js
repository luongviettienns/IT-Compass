...
export const getCurrentTemplate = async () => ({
    ...
    questionCount: assessmentCatalog.hollandQuestions.length + assessmentCatalog.situationalQuestions.length,
...
export const submitAttempt = async ({ userId, startedAt, hollandAnswers, situationalAnswers, }) => {
    const normalizedHolland = Object.fromEntries(HOLLAND_GROUPS.map((group) => [group, 0]));
    // Chỉ chấm điểm khi toàn bộ câu hỏi bắt buộc đã có mặt để frontend/backend luôn ra cùng một kết quả.
    const serializedHollandAnswers = assessmentCatalog.hollandQuestions.map((question) => {
    ...
    const serializedSituationalAnswers = assessmentCatalog.situationalQuestions.map((question) => {
    ...
    const attempt = await prisma.assessmentAttempt.create({
        data: {
            ...
            rawScoresJson: analysis.totalScores,
            answersJson: {
...
export const getAttemptHistory = async ({ userId, page, limit, }) => {
...
export const getAssessmentSummaryForUser = async ({ userId }) => {
