...
const getTokenExpiryDate = (durationMs) => {
    const expiresAt = new Date();
    expiresAt.setTime(expiresAt.getTime() + durationMs);
    return expiresAt;
};
const getEmailVerificationExpiryDate = () => getTokenExpiryDate(authConfig.emailVerificationTokenTtlHours * 60 * 60 * 1000);
const getPasswordResetExpiryDate = () => getTokenExpiryDate(authConfig.passwordResetTokenTtlMinutes * 60 * 1000);
// Token xác thực chỉ hợp lệ khi hash khớp, chưa bị dùng và vẫn còn trong thời hạn.
const buildActiveVerificationWhere = (token) => ({
    tokenHash: hashToken(token),
    usedAt: null,
    expiresAt: { gt: new Date() },
});
// Reset token cũng là token một lần dùng nên điều kiện active tương tự verify email.
const buildActiveResetWhere = (token) => ({
    tokenHash: hashToken(token),
    usedAt: null,
    expiresAt: { gt: new Date() },
});
...
export const findActiveEmailVerificationToken = async (token) => prisma.emailVerificationToken.findFirst({
...
export const markEmailVerificationUsedAndVerifyUser = async (verification) => prisma.$transaction([
...
export const findActivePasswordResetToken = async (token) => prisma.passwordResetToken.findFirst({
    where: buildActiveResetWhere(token),
});
export const consumeResetTokenAndUpdatePassword = async ({ resetToken, passwordHash, }) => prisma.$transaction([
