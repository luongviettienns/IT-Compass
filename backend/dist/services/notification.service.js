...
import { emitNotificationCreated, emitNotificationUpdated } from '../socket/notification.events.js';
...
const formatBookingDateTime = (startAt, endAt) => {
...
const isPrismaUniqueConstraintError = (error) => (typeof error === 'object' && error !== null && 'code' in error && error.code === 'P2002');
const createNotificationRecord = async (input, client = prisma) => {
    if (input.dedupeKey) {
...
const buildBookingNotificationData = (booking, extra = {}) => ({
...
const buildBookingNotificationTargets = (booking, type) => {
    ...
    const pushTarget = (targets, userId, title, body, dedupeSuffix) => {
        if (!userId)
            return;
        targets.push({
            userId,
    ...
    switch (type) {
        case 'BOOKING_REQUESTED':
            pushTarget(targets, booking.mentorUserId, 'Yêu cầu tư vấn mới', `${booking.studentName} đã ${requestTypeLabel} vào ${timeLabel}.`, 'requested');
            break;
        case 'BOOKING_CONFIRMED':
            pushTarget(targets, booking.studentUserId, 'Lịch tư vấn đã được xác nhận', `Mentor ${booking.mentorName} đã xác nhận lịch tư vấn vào ${timeLabel}. Bạn có thể mở chat ngay bây giờ.`, 'confirmed');
            break;
        case 'BOOKING_CANCELLED_BY_STUDENT':
            pushTarget(targets, booking.mentorUserId, 'Học viên đã hủy lịch', `${booking.studentName} đã hủy lịch tư vấn vào ${timeLabel}.${cancelReasonLabel}`, 'cancelled-by-student');
            break;
        case 'BOOKING_CANCELLED_BY_MENTOR':
            pushTarget(targets, booking.studentUserId, 'Mentor đã hủy lịch', `Mentor ${booking.mentorName} đã hủy lịch tư vấn vào ${timeLabel}.${cancelReasonLabel}`, 'cancelled-by-mentor');
            break;
        case 'BOOKING_COMPLETED':
            pushTarget(targets, booking.mentorUserId, 'Buổi tư vấn đã hoàn thành', `Buổi tư vấn với ${booking.studentName} vào ${timeLabel} đã được đánh dấu hoàn thành.`, 'completed');
            pushTarget(targets, booking.studentUserId, 'Buổi tư vấn đã hoàn thành', `Buổi tư vấn với mentor ${booking.mentorName} vào ${timeLabel} đã được đánh dấu hoàn thành.`, 'completed');
            break;
        case 'BOOKING_NO_SHOW':
            pushTarget(targets, booking.mentorUserId, 'Lịch tư vấn bị đánh dấu không tham dự', `Buổi tư vấn với ${booking.studentName} vào ${timeLabel} đã được đánh dấu không tham dự.`, 'no-show');
            pushTarget(targets, booking.studentUserId, 'Lịch tư vấn bị đánh dấu không tham dự', `Buổi tư vấn với mentor ${booking.mentorName} vào ${timeLabel} đã được đánh dấu không tham dự.`, 'no-show');
            break;
        case 'BOOKING_REMINDER':
            pushTarget(targets, booking.mentorUserId, 'Sắp đến giờ tư vấn', `Buổi tư vấn vào ${timeLabel} sẽ bắt đầu sau 30 phút.`, 'reminder-30m');
            pushTarget(targets, booking.studentUserId, 'Sắp đến giờ tư vấn', `Buổi tư vấn vào ${timeLabel} sẽ bắt đầu sau 30 phút.`, 'reminder-30m');
            break;
        default:
            break;
    }
...
const dispatchBookingNotifications = async (booking, type) => {
...
const toReminderBookingNotificationSource = (booking) => ({
...
export const notifyBookingRequested = async (booking) => {
...
export const notifyBookingCancelledByStudent = async (booking) => {
    await dispatchBookingNotifications(booking, 'BOOKING_CANCELLED_BY_STUDENT');
};
export const notifyBookingCancelledByMentor = async (booking) => {
...
export const notifyBookingNoShow = async (booking) => {
...
export const sendBookingReminderNotifications = async () => {
    ...
    await Promise.allSettled(reminderBookings.map((booking) => dispatchBookingNotifications(toReminderBookingNotificationSource(booking), 'BOOKING_REMINDER')));
