...
import {
  HOLLAND_GROUPS,
  QUIZ_SCORING_THRESHOLDS,
  RESULT_AFFINITY_WEIGHTS,
  RESULT_CODE_BY_PAIR,
  RESULT_MATCH_PERCENT,
  SPECIAL_ACTION_MATCH_BONUS,
...
const getPrimaryTraits = (resultCode: Exclude<AssessmentResultCode, 'Fallback'>): HollandGroup[] =>
  Object.entries(RESULT_AFFINITY_WEIGHTS[resultCode])
    .sort((a, b) => b[1] - a[1])
    .slice(0, 2)
    .map(([group]) => group as HollandGroup);

const buildResultDecision = (resultCode: AssessmentResultCode, fallbackTriggered: boolean) => ({
  resultCode,
  fallbackTriggered,
});
...
const resolveResultCode = ({
  ...
  const fallbackTriggered =
    top1.score < QUIZ_SCORING_THRESHOLDS.fallbackTopScore ||
  ...
  if (specialActions.includes('Data')) return buildResultDecision('Data', fallbackTriggered);
  if (specialActions.includes('UXUI')) return buildResultDecision('UXUI', fallbackTriggered);
  if (specialActions.includes('QLDA')) return buildResultDecision('QLDA', fallbackTriggered);

  return buildResultDecision('SE', fallbackTriggered);
};

const buildHollandBreakdown = (
...
const buildMajorRanking = (
  totalScores: HollandScores,
  specialActions: string[],
): MajorRankingItem[] => {
  const topScore = Math.max(...Object.values(totalScores), 1);
  const normalizedScores = Object.fromEntries(
    HOLLAND_GROUPS.map((group) => [group, totalScores[group] / topScore]),
  ) as HollandScores;

  const ranking = assessmentCatalog.resultOrder
    .filter((resultCode): resultCode is Exclude<AssessmentResultCode, 'Fallback'> => resultCode !== 'Fallback')
    .map((resultCode) => {
      const profile = assessmentCatalog.resultProfiles[resultCode];
      const affinityScore =
        HOLLAND_GROUPS.reduce(
          (sum, group) => sum + normalizedScores[group] * RESULT_AFFINITY_WEIGHTS[resultCode][group],
          0,
        ) + (specialActions.includes(resultCode) ? SPECIAL_ACTION_MATCH_BONUS : 0);
  ...
  return ranking.map((item) => {
    const matchPercent = Math.round(
      RESULT_MATCH_PERCENT.base + (item.affinityScore / topAffinityScore) * RESULT_MATCH_PERCENT.range,
    );
...
export const buildAssessmentSummary = ({
  ...
  hollandBreakdown,
  scoringMeta,
}: AssessmentAnalysis): AssessmentSummary => {
  const profile = assessmentCatalog.resultProfiles[resultCode];
