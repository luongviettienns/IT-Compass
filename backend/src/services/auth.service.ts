...
import type { AuthResponse, RequestMetadata } from '../types/auth.js';
import {
  createSession,
...
import {
  ...
  markEmailVerificationUsedAndVerifyUser,
} from './auth-token.service.js';
import { getAssessmentSummaryForUser } from './assessment.service.js';

interface RegisterInput extends RequestMetadata {
  fullName: string;
  email: string;
  password: string;
  role: 'STUDENT' | 'MENTOR';
...
interface RefreshInput extends RequestMetadata {
  refreshToken: string;
}
...
const withOptionalDebugToken = (token?: string) =>
  authConfig.debugExposeTokens && token ? { token } : {};
...
const assertActiveUser = (user: Pick<User, 'status'>) => {
...
export const login = async ({ email, password, userAgent, ipAddress }: LoginInput) => {
  const normalizedEmail = sanitizeEmailAddress(email);
...
export const refresh = async ({ refreshToken, userAgent, ipAddress }: RefreshInput) => {
...
export const logout = async ({ refreshToken }: { refreshToken: string | null }) => {
  if (!refreshToken) {
    return;
  }
...
export const logoutAll = async ({ userId }: { userId: bigint }) => {
  await revokeAllSessionsForUser(userId);
};

export const me = async ({ userId }: { userId: bigint }) => {
...
export const requestVerifyEmail = async ({ userId }: { userId: bigint }) => {
...
export const confirmVerifyEmail = async ({ token }: { token: string }) => {
