import type { BookingRequestType, BookingStatus, Prisma } from '@prisma/client';
...
import {
  notifyBookingCancelledByMentor,
  notifyBookingCancelledByStudent,
...
const ACTIVE_BOOKING_STATUSES: BookingStatus[] = ['REQUESTED', 'CONFIRMED'];
...
const bookingSettingSelect = {
  ...
  autoConfirm: true,
} satisfies Prisma.MentorBookingSettingSelect;

type BookingSettingRecord = Prisma.MentorBookingSettingGetPayload<{ select: typeof bookingSettingSelect }>;
...
const DEFAULT_BOOKING_SETTING = {
  ...
  bufferAfterMinute: 0,
  autoConfirm: false,
} satisfies BookingSettingRecord;
...
const minuteToTime = (minute: number) => `${pad2(Math.floor(minute / 60))}:${pad2(minute % 60)}`;
...
const parseDateBoundary = (date: string, mode: 'start' | 'end') => {
  const [yearPart, monthPart, dayPart] = date.split('-');
  const year = Number(yearPart);
  const monthIndex = Number(monthPart) - 1;
  const day = Number(dayPart);
...
const normalizePagination = (query: ListBookingsQuery) => ({
  page: toPositiveInteger(query.page, DEFAULT_PAGE),
  limit: toPositiveInteger(query.limit, DEFAULT_LIMIT),
});

const serializeSetting = (setting: BookingSettingRecord) => ({
...
const buildDurationOptions = (setting: BookingSettingRecord) => {
...
const toBookingNotificationSource = (booking: BookingRecord): BookingNotificationSource => ({
...
const sendBookingNotificationSafely = async (
  notificationTask: () => Promise<unknown>,
  context: { bookingId: bigint; event: string },
) => {
  try {
...
const logBookingTransition = ({
  ...
  actorType: 'STUDENT' | 'MENTOR' | 'SYSTEM';
  action: 'CREATE' | 'CONFIRM' | 'CANCEL' | 'COMPLETE' | 'NO_SHOW';
...
const getBookingSettingOrDefault = async (mentorId: bigint): Promise<BookingSettingRecord> => {
...
const getSelfMentor = async (userId: bigint) => {
...
const getPublicMentorForBooking = async (slug: string) => {
...
const assertDurationAllowed = (durationMinute: number, setting: BookingSettingRecord) => {
...
const assertBookingWindowAllowed = (startAt: Date, setting: BookingSettingRecord) => {
  const now = new Date();
...
const assertBookingAbuseLimits = async (
  tx: TransactionClient | typeof prisma,
  {
    studentUserId,
    mentorId,
...
const validateSetting = (setting: BookingSettingRecord) => {
  ...
  if (
    setting.defaultDurationMinute < setting.minDurationMinute ||
...
const validateAvailabilityBlocks = (blocks: UpdateMentorAvailabilityBody['blocks']) => {
  const activeBlocksByWeekday = new Map<number, Array<{ startMinute: number; endMinute: number }>>();
...
const getBookingDetailOrThrow = async (where: Prisma.MentorBookingWhereInput, tx: TransactionClient | typeof prisma = prisma) => {
...
export const getPublicAvailability = async ({
  ...
  const slots = blocks.flatMap((block) => {
    ...
    for (
      ...
      startMinute + durationMinute <= block.endMinute;
      startMinute += DURATION_STEP_MINUTE
    ) {
...
export const createBooking = async ({
  ...
  const setting = await getBookingSettingOrDefault(mentor.id);
  const requestType: BookingRequestType = input.requestType;
...
export const cancelStudentBooking = async ({
  userId,
  bookingId,
  input,
}: {
...
export const getMentorAvailability = async ({ userId }: { userId: bigint }) => {
...
export const updateMentorAvailability = async ({
  ...
  userId: bigint;
  input: UpdateMentorAvailabilityBody;
}) => {
...
export const getMentorBookingSettings = async ({ userId }: { userId: bigint }) => {
  const mentor = await getSelfMentor(userId);
...
export const updateMentorBookingSettings = async ({
  userId,
  input,
}: {
  userId: bigint;
  input: UpdateMentorBookingSettingsBody;
}) => {
  const mentor = await getSelfMentor(userId);
...
export const listMentorBookings = async ({
  userId,
  query,
}: {
  userId: bigint;
...
export const confirmMentorBooking = async ({ userId, bookingId }: { userId: bigint; bookingId: bigint }) => {
  const mentor = await getSelfMentor(userId);
  const setting = await getBookingSettingOrDefault(mentor.id);
...
export const cancelMentorBooking = async ({
  userId,
  bookingId,
  input,
}: {
...
export const markOverdueBookingsAsNoShow = async () => {
  ...
  if (overdueBookings.length === 0) return { marked: 0 };

  const updatedBookings = await prisma.$transaction(async (tx) => Promise.all(
    overdueBookings.map((booking) => tx.mentorBooking.update({
      where: {
        id: booking.id,
        status: 'CONFIRMED',
      },
