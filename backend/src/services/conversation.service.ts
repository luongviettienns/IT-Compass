...
const normalizeConversationPagination = (query: ListConversationsQuery) => ({
  page: toPositiveInteger(query.page, DEFAULT_PAGE),
  limit: toPositiveInteger(query.limit, DEFAULT_CONVERSATION_LIMIT),
});

const normalizeMessagePagination = (query: ListConversationMessagesQuery) => ({
  page: toPositiveInteger(query.page, DEFAULT_PAGE),
  limit: toPositiveInteger(query.limit, DEFAULT_MESSAGE_LIMIT),
});

const serializeBookingSummary = (booking: ConversationRecord['booking']) => ({
...
const serializeConversation = (conversation: ConversationRecord) => ({
...
const canAccessConversation = (conversation: { mentor: { userId: bigint | null }; studentUserId: bigint }, userId: bigint) => (
  conversation.studentUserId === userId || conversation.mentor.userId === userId
);

const canSendConversationMessage = (booking: { status: string }) => booking.status === 'CONFIRMED';

export const ensureConversationForConfirmedBooking = async (tx: TransactionClient, booking: {
  id: bigint;
  mentorId: bigint;
...
export const getConversationForAccess = async (conversationId: bigint, userId: bigint) => {
...
export const listConversations = async ({ userId, query }: { userId: bigint; query: ListConversationsQuery }) => {
...
export const getConversationByBooking = async ({ userId, bookingId }: { userId: bigint; bookingId: bigint }) => {
...
export const listMessages = async ({
  ...
  const where = { conversationId } satisfies Prisma.MessageWhereInput;
...
export const createMessage = async ({
  ...
  userId: bigint;
  conversationId: bigint;
  input: CreateConversationMessageBody;
}) => {
