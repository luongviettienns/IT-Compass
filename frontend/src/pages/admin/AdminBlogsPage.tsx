...
export default function AdminBlogsPage() {
    ...
    const deleteMutation = useMutation({
        ...
        onError: (error) => toast.error(getErrorMessage(error, 'Không thể xóa bài viết.')),
    });
    ...
    const restoreMutation = useMutation({
        ...
        onError: (error) => toast.error(getErrorMessage(error, 'Không thể khôi phục bài viết.')),
    });
    ...
    const publishMutation = useMutation({
        ...
        onError: (error) => toast.error(getErrorMessage(error, 'Không thể xuất bản bài viết.')),
    });
    ...
    const previewMutation = useMutation({
        ...
        onError: (error) => toast.error(getErrorMessage(error, 'Không thể mở bản xem trước.')),
    });
    ...
    const bulkDeleteMutation = useMutation({
        ...
        onError: (error) => toast.error(getErrorMessage(error, 'Không thể xóa các bài viết đã chọn.')),
    });
    ...
    const bulkRestoreMutation = useMutation({
        ...
        onError: (error) => toast.error(getErrorMessage(error, 'Không thể khôi phục các bài viết đã chọn.')),
    });
    ...
    const bulkPublishMutation = useMutation({
        ...
        onError: (error) => toast.error(getErrorMessage(error, 'Không thể xuất bản các bài viết đã chọn.')),
    });
    ...
    const confirmAction = () => {
        ...
        if (actionDialog.type === 'bulkDelete' || actionDialog.type === 'bulkRestore' || actionDialog.type === 'bulkPublish') {
            if (!selectedPostIds.length) {
                toast.error('Vui lòng chọn ít nhất một bài viết.');
                return;
            }
    ...
    return (
        <>
            <Helmet><title>Quản lý Bài viết — Admin — IT Compass</title></Helmet>
...

                {
                    selectedPostIds.length > 0 && (
                        <div className="bg-primary/10 border-primary border p-4 rounded-2xl flex items-center justify-between animate-in fade-in slide-in-from-top-4 shadow-sm">
                            ...
                            <div className="flex items-center gap-2">
                                <button disabled={isMutating} onClick={() => openBulkActionDialog('bulkPublish')} className="text-xs font-bold px-3 py-1.5 bg-emerald-500 text-white flex items-center gap-1 rounded-lg hover:scale-105 active:scale-95 transition-all"><Send className="w-3 h-3" /> Xuất bản</button>
                                <button disabled={isMutating} onClick={() => openBulkActionDialog('bulkRestore')} className="text-xs font-bold px-3 py-1.5 bg-blue-500 text-white flex items-center gap-1 rounded-lg hover:scale-105 active:scale-95 transition-all"><RefreshCcw className="w-3 h-3" /> Khôi phục</button>
                                <div className="w-px h-6 bg-primary/20 mx-2"></div>
                                <button disabled={isMutating} onClick={() => openBulkActionDialog('bulkDelete')} className="text-xs font-bold px-3 py-1.5 bg-destructive text-white flex items-center gap-1 hover:bg-destructive/90 rounded-lg hover:scale-105 active:scale-95 transition-all"><Trash2 className="w-3 h-3" /> Xóa</button>
                            </div>
                        </div>
                    )
                }
