...
export default function AdminUsersPage() {
    ...
    const statusMutation = useMutation({
        ...
        onError: (error) => toast.error(getErrorMessage(error, 'Không thể cập nhật trạng thái người dùng.')),
    });
    ...
    const bulkStatusMutation = useMutation({
        ...
        onError: (error) => toast.error(getErrorMessage(error, 'Không thể cập nhật trạng thái hàng loạt.')),
    });
    ...
    const validateActionReason = (reason: string) => {
        const normalizedReason = reason.trim();
        if (normalizedReason.length < 3) {
            toast.error('Lý do thực hiện thao tác cần ít nhất 3 ký tự.');
            return false;
        }
        if (normalizedReason.length > 500) {
            toast.error('Lý do thực hiện thao tác không được vượt quá 500 ký tự.');
            return false;
        }
        return true;
    };
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease: [0.22, 1, 0.36, 1] }} className="space-y-6">
                ...
                <AdminActionDialog
                    ...
                    onConfirm={(reason) => {
                        ...
                        statusMutation.mutate({ id: statusDialog.id, status: statusDialog.status, reason: reason.trim() });
                    }}
                />
                <AdminActionDialog
                    ...
                    onConfirm={(reason) => {
                        ...
                        if (!validateActionReason(reason)) return;
                        bulkStatusMutation.mutate({ userIds: selectedUserIds, status: bulkStatusDialog.status, reason: reason.trim() });
                    }}
                />
                <AdminActionDialog
